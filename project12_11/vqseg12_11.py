...
class Segmentor(nn.Module):
    ...
    def forward(self,x,return_all= False):
        with torch.no_grad():
            ...
            #print("dense feature size(): ",dense_feature.size())#14*14
            #print("dense_feature: ",dense_feature.dtype)
            dense_feature = dense_feature.reshape(-1,14,14,768)
            dense_feature = dense_feature.permute(0,3,1,2)
            #print("dense feature size(): ",dense_feature.size())#14*14
            
            #features = self.proj(dense_feature)
            
            #print("features size(): ",features.size())#7*7
            #dense_feature.to(torch.float32)

            #features = self.v_proj(dense_feature)
            #features = self.c_proj(features)
            #print("dense features size(): ",feature.size())
    ...
    def validate(self, num_iter: int) -> None:
        ...
        for num_batch in pbar:
            ...
            '''
                ...
                gt.size():  (16, 448, 448)
                dt shape:  torch.Size([16, 128, 448, 448])
            '''
        ...
        with Pool(16) as pool:
            for num_batch in pbar:
                dict_data = next(iter_dataloader)
                '''
                    second dt shape:  torch.Size([16, 128, 448, 448])
                    img_ size:  torch.Size([16, 3, 448, 448])
                    gt shape: (16, 448, 448)
                    dt_crf_argmax shape: (16, 448, 448)
                '''
                img: torch.Tensor = dict_data["img"].to(self.device)  # b x 3 x H x W
                ...
                img_=F.interpolate(img,size=(448,448),mode='bilinear')
                #print("img_ size: ",img_.shape)
                dt_crf_argmax = batched_crf(pool, img_, torch.log_softmax(dt, dim=1)).argmax(1).to(self.device)  # b x H x W
