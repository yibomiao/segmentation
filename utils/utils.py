...
class Upsampling_neck(nn.Module):
    def __init__(self, in_dim):
        super(Upsampling_neck,self).__init__()
        self.in_dim = in_dim
        ...
        #self.upsample1 = nn.ConvTranspose2d(self.in_dim, self.in_dim//2, kernel_size=3,stride=2,padding=1,output_padding=1)
        #self.bn1 = nn.BatchNorm2d(self.in_dim//2),
        #self.upsample2 = nn.ConvTranspose2d(self.in_dim//2, self.in_dim//4, kernel_size=3,stride=2,padding=1,output_padding=1)
        #self.bn2 = nn.BatchNorm2d(in_dim//4)
        #self.upsample3 = nn.ConvTranspose2d(self.in_dim//4, self.in_dim//8, kernel_size=3,stride=2,padding=1,output_padding=1)
        #self.upsample4 = nn.ConvTranspose2d(self.in_dim//8, self.in_dim//16, kernel_size=3,stride=2,padding=1,output_padding=1)
    
    def forward(self,input):
        up_1 = self.upsample1(input)
        up_2 = self.upsample2(up_1)
        up_3 = self.upsample3(up_2)
        up_4 = self.upsample4(up_3)
        up_5 = self.upsample5(up_4)
        return up_5

def upsampling_test(features:torch.Tensor):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    #b,d,h,w stand for batch size,dimension,height,width respectively 
    b = features.size()[0]
    d = features.size()[1]
    h = features.size()[2] 
    w = features.size()[3]
    features=features.to(device)
    upsample1 = nn.ConvTranspose2d(d, d//2, kernel_size=3, stride=2, padding=1)
    upsample2 = nn.ConvTranspose2d(d//2, d//4, kernel_size=3, stride=2, padding=1)
    upsample1 = upsample1.to(device)
    upsample2 = upsample2.to(device)
    output1 = upsample1(features)
    output2 = upsample2(output1)
