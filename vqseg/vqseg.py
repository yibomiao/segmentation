...
class VqSeg:
    ...
    def __call__(
        ...
        for num_iter in pbar:
            ...
            # backward
            loss_recons = F.mse_loss(feature_rec, feature)
    ...
    @torch.no_grad()
    def myvalidate(self, num_iter: int) -> None:
        ...
        for num_batch in pbar:
            ...
            dt_argmax: torch.Tensor = self.segmentor(img)  # b x n_cats x H x W
            #dt_argmax为index
            print("dt_argmax.size(): ",dt_argmax.size())
        ...
        print(
            ...
            f"mIoU: {miou:.3f} ({miou:.3f}) | "
            f"pixel acc.: {pixel_acc:.3f} ({pixel_acc:.3f})"
        )
